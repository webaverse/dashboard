...
const Mounts = () => {
  ...
  const handleFilesMagically = async (files) => {
    ...
    if (files.length === 1) {
      ...
      alert("Please only upload 1 glb file for mounts.");
      setLoading(false);
    }
  }

  const makeMountWbn = async (file) => {
    if (file && getExt(file[0].name) === "glb") {
      const sitBone = prompt("What is the name of the sit bone for this mount", "");
      const walkAnimation = prompt("What is the name of the walk animation for this mount?", "");
      const manifest = {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        .then(data => {
          ...
          router.push('/preview/' + data.hash + "." + fileName + "." + extName + ".mounts");
        })
        .catch(error => {
          console.error(error)
        })
